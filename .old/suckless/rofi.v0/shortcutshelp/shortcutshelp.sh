...
keybindings=$(awk '
    ...
    { last="" } 
    /^#/ { last=substr($0, 3) }' ~/.config/bspwm/sxhkd/sxhkdrc)
...
selected=$(echo "$formatted_keybindings" | rofi -dmenu -i -p "Keybindings" -line-padding 4 -hide-scrollbar -theme ~/.config/bspwm/rofi/shortcutshelp/style.rasi)
